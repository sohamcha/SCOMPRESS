...
node * initdict(node *head,char c)
{
    node *temp;
   // int value1=97;
  //  int value2=65;
...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   cout<<"***** SCOMPRESS - the General Purpose Compression Tool ( V 0.6 Beta DATED : 7/17/2011 Compiled in VC++ 2010 / .NEt 4 ) ***** \n\n - Soham Chakraborty \n\n";
   ...
   if(choice==1)
   {
      ...
      //  file.open("REDUNDANT-File.txt",ios::in|ios::out);                           // For Compression Ratio performance testing  ----> Observation : Performance is lower if Burrows Wheeler Transform is applied to LZW
        ...
        file2.open("OUTPUT-LZW.dat",ios::out|ios::binary);
        ...
        while(file.good())
        {
           ...
           prev=searchprev(head,&tempstring);
            ...
            if(prev==NULL)
            {
                ...
                tail=temp;                //  CHANGED UPTO <------- HERE ( 7.21.2011 )
                
                if((tempstring.length()==1)&&(prev!=NULL))
                    file2<<prev->val;           // CHANGE REQ.
                if((tempstring.length()>1)&&(prev!=NULL))
                {
          
                 file2<<prev->index;           // Change Required
        ...
        cout<<"\nFINAL STR : "<<tempstring;      
           if(tempstring.length()==1)
                     file2<<tempstring[0];        // change req.
                 
           if(tempstring.length()>1)
                 {
                     prev=searchprev(head,&tempstring);
                     file2<<prev->index;           //  Change req.
     ...
     cin>>rle;
        ...
        if(rle==1)
        {
          ...
          if(i<tempstring.length())
          {
              ...
              ch=tempstring[i-1];
              i++;
              while((tempstring[i]!='$')&&(i<tempstring.length()))
              {
           ...
           // cout<<"\nNo. of occurence of "<<ch<<" is "<<count;
            finalstr.resize(finalstr.length()-1);                        //   REMOVING THE EXTRA COPY
            for(j=0;j<count;j++)
